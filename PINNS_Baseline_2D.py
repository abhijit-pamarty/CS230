...
def boundary_x0(x, on_boundary):
    return on_boundary and np.isclose(x[0], 0, atol=0.1)

def boundary_x1(x, on_boundary):
    return on_boundary and np.isclose(x[0], 1, atol=0.1)

def boundary_y0(x, on_boundary):
    return on_boundary and np.isclose(x[1], 0, atol=0.1)

def boundary_y1(x, on_boundary):
    return on_boundary and np.isclose(x[1], 1, atol=0.1)


def boundary_y(x, on_boundary):
    return on_boundary and (np.isclose(x[1], 0, atol=0.1) or np.isclose(x[1], 1, atol=0.1))

def initial_condition(x):
    return np.isclose(x[:, 0], 0, atol=0.1).astype(np.float32)

def solution_func(x):
    return scaled_1.flatten()[5000:10000]

# Concentration from dataset
#n_obs = time0.shape
#observe_xt = np.vstack((np.ones(n_obs), np.array(time0))).T
#observe_c = li_conc0.reshape(-1,1)
observe_pts = dde.icbc.PointSetBC(data_xyt[5000:10000], scaled_1.flatten()[5000:10000], component=0)
...
losshistory_lbfgs, train_state_lbfgs = model.train(iterations=30000)
...
im = plt.imshow(C_pred_2, cmap='viridis', vmin=np.min(C_pred/np.max(C_pred)), vmax=1)
